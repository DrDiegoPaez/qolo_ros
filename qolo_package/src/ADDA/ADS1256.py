...
class ADS1256:
    def __init__(self, adda, rst_pin, cs_pin, drdy_pin):
        self.adda = adda
    ...
    def ADS1256_reset(self):
        self.adda.digital_write(self.rst_pin, self.adda.HIGH)
        self.adda.delay_ms(200)
        self.adda.digital_write(self.rst_pin, self.adda.LOW)
        self.adda.delay_ms(200)
        self.adda.digital_write(self.rst_pin, self.adda.HIGH)
    
    def ADS1256_WriteCmd(self, reg):
        self.adda.digital_write(self.cs_pin, self.adda.LOW)#cs  0
        self.adda.spi_writebyte([reg])
        self.adda.digital_write(self.cs_pin, self.adda.HIGH)#cs 1
    
    def ADS1256_WriteReg(self, reg, data):
        self.adda.digital_write(self.cs_pin, self.adda.LOW)#cs  0
        self.adda.spi_writebyte([CMD['CMD_WREG'] | reg, 0x00, data])
        self.adda.digital_write(self.cs_pin, self.adda.HIGH)#cs 1
        
    def ADS1256_Read_data(self, reg):
        self.adda.digital_write(self.cs_pin, self.adda.LOW)#cs  0
        self.adda.spi_writebyte([CMD['CMD_RREG'] | reg, 0x00])
        data = self.adda.spi_readbytes(1)
        self.adda.digital_write(self.cs_pin, self.adda.HIGH)#cs 1
    ...
    def ADS1256_WaitDRDY(self):
        for i in range(0,400000,1):
            if(self.adda.digital_read(self.drdy_pin) == 0):
                break
        if(i >= 400000):
            print ("Time Out ...\r\n")      
    ...
    def ADS1256_ConfigADC(self, gain, drate):
        ...
        self.adda.digital_write(self.cs_pin, self.adda.LOW)#cs  0
        self.adda.spi_writebyte([CMD['CMD_WREG'] | 0, 0x03])
        self.adda.spi_writebyte(buf)
        
        self.adda.digital_write(self.cs_pin, self.adda.HIGH)#cs 1
        self.adda.delay_ms(1)
    ...
    def ADS1256_Read_ADC_Data(self):
        self.ADS1256_WaitDRDY()
        self.adda.digital_write(self.cs_pin, self.adda.LOW)#cs  0
        self.adda.spi_writebyte([CMD['CMD_RDATA']])
        # self.adda.delay_ms(10)

        buf = self.adda.spi_readbytes(3)
        self.adda.digital_write(self.cs_pin, self.adda.HIGH)#cs 1
